...
is default-deny with one Allow policy containing the explicit Google email
addresses authorized for Miso; no broad domain or Everyone selector is used.
Google is the only enabled login method for the app. This policy is the sole
remote membership allowlist.
...
email. A verified email becomes the Miso web actor; email authorization is
delegated to the Cloudflare Access policy. Supplying only the
`Cf-Access-Authenticated-User-Email` header cannot authenticate a request.
...
account must be denied by Cloudflare Access, and an assertion for a different
Access application must be denied by Miso. Direct origin probes with a forged
identity header must receive `401`.
